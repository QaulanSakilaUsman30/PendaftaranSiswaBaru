...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $query_delete = mysqli_query($conn, "DELETE FROM administrasi WHERE Id_Administrasi = '$id_administrasi'");

    if ($query_delete) {
        // Jika penghapusan berhasil, redirect kembali ke halaman daftar siswa
        header('location: index.php?ke=administrasi'); // Sesuaikan path jika perlu
        exit();
    } else {
